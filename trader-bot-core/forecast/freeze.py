...
# PKT-TRADER-BOT-FORECAST-SPINE-RELOCATE (P3): this module lives at
# trader-bot-core/forecast/freeze.py. The engine is a first-class sibling
# package (trader-bot-core/engine); the model artifacts are relocated verbatim
# under forecast/ref/p7proto/models_out_007. engine_sha (basename+bytes) and
# model_sha (concatenated bytes) are byte-preserved by the verbatim relocation.
_THIS = Path(__file__).resolve()
_CORE_ROOT = _THIS.parents[1]                  # trader-bot-core
_ENGINE_DIR = _CORE_ROOT / "engine"            # verbatim engine package
_LOCAL_MODEL_ROOT = _THIS.parent / "ref" / "p7proto" / "models_out_007"
_REPO_ROOT = _THIS.parents[2]                  # repo root (checkout fallback)
...
def resolve_model_root() -> Optional[Path]:
    # First-class relocated root wins (trader-bot-core/forecast/ref/p7proto).
    if (_LOCAL_MODEL_ROOT / MODEL_FILES_ORDERED[0]).exists():
        return _LOCAL_MODEL_ROOT
    root = _freeze_root()
