...
# PKT-TRADER-BOT-FORECAST-SPINE-RELOCATE (P3): first-class paths only. The
# forecast spine now lives at trader-bot-core/forecast/; there is NO runtime
# sys.path.append(runs/pkt_tb_00X/...) — the prototype-dir-as-live-runtime trap
# (architecture invariant 5) is removed. The frozen reference data (dicts,
# nightly_007 parity refs, model artifacts) is relocated verbatim under ref/;
# the frozen seed caches live under store/seeds/ (SEED_ROOT).
SHADOW = Path(__file__).resolve().parent            # trader-bot-core/forecast
CORE_ROOT = SHADOW.parent                           # trader-bot-core
REPO = CORE_ROOT                                    # config/universe.csv root
PROTO7 = SHADOW / "ref" / "p7proto"                 # dicts/, store/nightly_007/, models_out_007/
TB6_PROTO = SHADOW / "ref" / "tb6proto"             # feature_store proto_dir (dicts + universe via parents[2])
SEED_ROOT = CORE_ROOT / "store" / "seeds"           # frozen seed caches (cache/*, gdelt_cache/daily, store/llm_features)
...
# STATE is the brain's WRITABLE working tree. Locally it defaults to
# forecast/_state (gitignored); inside the read-only Lambda image it is
# redirected to /tmp via BRAIN_STATE_DIR (set by decide.cutover.production_
# forecaster), with the baked read-only seed caches copied in at cold start.
STATE = Path(os.environ.get("BRAIN_STATE_DIR") or (SHADOW / "_state"))
...
def s3_client():
    import boto3
    # PKT-TRADER-BOT-FORECAST-SPINE-RELOCATE (P3): the laptop AWS_PROFILE
    # assumption is stripped — always use the default credential chain (the
    # Lambda execution role in prod; the caller's default creds locally). The
    # relocated spine never assumes a named 'personal' profile.
