...
from .shadow_lib import (CACHE_CBOE, CACHE_COT, CACHE_DAILY, CACHE_FRED,
                         ...
                         GDELT_RECORDS_DIR, ORGAN_DIR, PROTO7, SEED_ROOT,
...
def ensure_seed_caches(logf: Optional[Path] = None) -> None:
    """One-time copy of the frozen seed caches into the writable STATE tree.
    Seeds are relocated first-class under store/seeds/ (SEED_ROOT); the STATE
    tree (/tmp in Lambda) stays runtime-writable."""
    import shutil
    seeds = [
        (SEED_ROOT / "cache" / "ohlcv", CACHE_OHLCV),
        (SEED_ROOT / "cache" / "cboe", CACHE_CBOE),
        (SEED_ROOT / "cache" / "fred", CACHE_FRED),
        (SEED_ROOT / "cache" / "cot", CACHE_COT),
        (SEED_ROOT / "gdelt_cache" / "daily", GDELT_DAILY_DIR),
    ]
...
def gdelt_forward(logf: Optional[Path] = None) -> int:
    ...
    masking covers it."""
    from . import gdelt_backfill as GB
    from . import features_gdelt as FG
...
def build_p7_extras(panel: dict) -> dict:
    ...
    from . import make_targets_007 as MT
