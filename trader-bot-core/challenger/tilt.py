...
CORE_ROOT = PROTO.parents[0]                           # trader-bot-core (first-class)

# First-class-path port (PKT-TRADER-BOT-SEED-CANON-BY-REPLAY-V2 §challenger): the
# transitive deps are relocated into trader-bot-core/challenger/ verbatim; the
# legacy prototype's runtime ``sys.path.insert(runs/...)`` hack is REMOVED (no
# runtime dependency on runs/pkt_tb_00*/). Logic below is unchanged.
from challenger.genome import Genome007
from challenger.lot_fix import aggregate_book
from challenger.risk_stats import RiskStats
from challenger.strategy import Strategy, StrategyContext
...
def make_tilt_strategy(genome: Genome007 | dict | str | Path,
                       ...
                       universe_csv: Path | str = CORE_ROOT / "config" / "universe.csv",
