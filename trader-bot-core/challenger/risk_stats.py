...
Cache source (first-class port, PKT-TRADER-BOT-SEED-CANON-BY-REPLAY-V2): the core
OHLCV store — the challenger driver passes the EXTENDED store (seed + spliced
post-split bars) so beta/sigma are as-of-D. NO runtime dependency on runs/.
"""
...
CORE_ROOT = PROTO.parents[0]                            # trader-bot-core
# First-class-path port (PKT-TRADER-BOT-SEED-CANON-BY-REPLAY-V2): the default
# OHLCV source is the core seed store — NO runtime dependency on runs/pkt_tb_006/.
# The seed closes cross-check the live S3 daily snapshots; adj_close carries the
# split/dividend-safe series. The challenger driver overrides ohlcv_dir with the
# EXTENDED store (seed + spliced post-split bars) so beta/sigma are as-of-D.
TB006_OHLCV = CORE_ROOT / "store" / "seeds" / "cache" / "ohlcv"
