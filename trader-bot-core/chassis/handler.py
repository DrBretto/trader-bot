...
from chassis.signals.compute_signals import run as compute_signals
from chassis.utils.market_calendar import latest_settled_session, settled_day_from_prices
from chassis.utils.s3_client import S3Client
from chassis.utils.logging_utils import setup_logger, log_step, StepTimer
...
# CU-05: config loader relocated out of this monolith so app.morning / app.midday
# can load config without importing chassis.handler. Re-imported here for the
# night phase + diag branches that still live in this module.
from chassis.config_loader import get_secret, load_config_from_s3
...
def _run_forecast_diag(event: dict, bucket: str, region: str) -> dict:
    ...
    from chassis.brain import diagnose_forecast_freshness
...
def _run_regime_diag(event: dict, bucket: str, region: str) -> dict:
    ...
    from chassis.brain import diagnose_regime_chassis
    pending = event.get('pending')
    if isinstance(pending, str):
        pending = [pending]
...
def _run_date_grid_diag(event: dict, bucket: str, region: str) -> dict:
    ...
    from chassis.canon.equity_append import _cache_tail
...
def _run_healthcheck(event: dict, bucket: str, region: str) -> dict:
    ...
    """
    from chassis.brain import monitors
    s3 = S3Client(bucket, region)
...
def _run_republish_dashboard(event: dict, bucket: str, region: str) -> dict:
    ...
    """
    from chassis.steps.publish_artifacts import (
        build_dashboard_data, _build_snapshot_meta, _can_publish_dashboard,
        _verify_ledger_or_hold,
    )
    from chassis.utils.dashboard_metrics import attach_new_brain_surface, sanitize_nan_for_json
