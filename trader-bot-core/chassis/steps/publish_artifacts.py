...
def _load_chart_markers() -> List[Dict[str, Any]]:
    ...
    candidates.append(Path(__file__).resolve().parents[3] / 'config' / 'chart_markers.json')
    for p in candidates:
        if p.is_file():
            try:
                with p.open() as f:
...
def run_brain_cutover_night(
    ...
    try:
        from chassis.brain import is_live, load_brain_config, production_forecaster, run_cutover
    ...
    from chassis.brain.model_registry import resolve_live_engine, EngineRegistryError
    try:
    ...
    if engine_name == "tilt_adapter":
        from chassis.brain.tilt_live import run_tilt_cutover
