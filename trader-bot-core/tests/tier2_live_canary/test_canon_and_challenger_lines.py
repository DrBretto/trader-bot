"""Tier 2 live-canary — the canon line is the ledger holdings×settled line (NOT
the ~$97k sim book) and the challenger line is populated.
...
sim book. It reads the live clean-ledger terminal + the live
...
``challenger_line_present_nonempty`` asserts the shadow/challenger artifact
...
def test_canon_line_is_not_sim_book(reality):
    term = reality.canon_terminal()                     # live clean-ledger leaf
    ...
    assert model.startswith("replay@") or "FREEZE_ORB1" in model, (
        f"canon terminal model_id {model!r} is not a replay/holdings-marked leaf")
...
@pytest.mark.live_canary
def test_challenger_line_present_nonempty(reality):
    ts = reality.shadow_timeseries()
    series = ts.get("shadow_A")
    assert isinstance(series, list) and len(series) >= 1, (
        f"challenger (shadow_A) missing or empty: {type(series).__name__}")

    last = series[-1]
    assert isinstance(last, (list, tuple)) and len(last) >= 2, f"malformed challenger point {last!r}"
    last_date, last_val = str(last[0]), float(last[1])
    assert last_val > 0, f"challenger last value non-positive: {last_val}"
    ...
    assert last_date == expected, (
        f"challenger last point {last_date} != expected settled day {expected} "
        f"(challenger is stale / not advancing)")
