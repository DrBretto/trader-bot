...
| 1.2 | Baseline-block second read (M-01/M-02/M-13) — **now the FIRST genuine test of the correctly trained models** | One quarter of stored inference from the corrected 2026-06-06 models accrues (this run's deep-era days were all short-corpus-mistrained output; the corrected models have 1 stored day) | Same harness (pilots/pilot_baseline_check.py), same arms, new window, conditioned on `model_versions >= *_v20260606`; this read — not this run's tie — carries the torch-path keep/kill decision |
