...
4. Simulates the fill for each validated trade at the morning open via `paper_trader` (fractional shares). There is no broker — the simulated fill is the new forward point on the line. Dust positions (< 0.001 shares from prior partial fills) are skipped gracefully.
5. Updates local portfolio state from the simulated fills (no broker reconcile).
